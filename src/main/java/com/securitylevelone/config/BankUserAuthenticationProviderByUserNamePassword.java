package com.securitylevelone.config;
...
@Component
public class BankUserAuthenticationProviderByUserNamePassword implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if(users.size() > 0){
            if(passwordEncoder.matches(pwd,users.get(0).getPassword()))
            {
                ...
                authorities.add(new SimpleGrantedAuthority(users.get(0).getUserRole()));
